...
*** @param *big_i
	struct BigInt big_int;nt The converted string now as BigInt.
...
*** multiply(), print_big_int()
	struct BigInt big_int;, and copy_big_int() consecutively from 2 to
...
void divide(const struct BigInt * big_int, int divisor, struct BigInt * big_result)
{
	...
	big_result->digits_count -= count_of_zeros;
}
